...
pub fn service(input: TokenStream) -> TokenStream {
    ...
    if features.methods {
        quote! {
            #[async_trait]
            impl krossbar_service::Service for Pin<Box<#struct_ident>>
                where Self: krossbar_service::service::ServiceMethods {
        ...
        quote! {
            #[async_trait]
            impl krossbar_service::Service for #struct_ident {
                fn service_name() -> &'static str {
                    #service_name
                }
...
pub fn service_impl(_attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        #[async_trait]
        impl krossbar_service::service::ServiceMethods for Pin<Box<#self_name>> {
...
pub fn peer(input: TokenStream) -> TokenStream {
    ...
    let peer_impl = if features.subscriptions {
        quote! {
            #[async_trait]
            impl krossbar_service::peer::Peer for Pin<Box<#struct_ident>>
                where Pin<Box<#struct_ident>>: krossbar_service::peer::PeerSignalsAndStates {
    ...
    quote! {
        impl krossbar_service::peer::PeerName for Pin<Box<#struct_ident>> {
...
pub fn peer_impl(_attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        #[async_trait]
        impl krossbar_service::peer::PeerSignalsAndStates for Pin<Box<#self_name>>
            where Pin<Box<#self_name>>: krossbar_service::peer::PeerName {
            async fn register_callbacks(&mut self, service_name: &str) -> krossbar_bus_lib::Result<()> {
                let context = krossbar_service::this::This { pointer: self };
